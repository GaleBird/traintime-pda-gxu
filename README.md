...
本项目基于 [BenderBlog/traintime_pda](https://github.com/BenderBlog/traintime_pda)（Traintime PDA / XDYou）继续维护，但已是一条**独立的维护线**，不是上游的简单分支：

- **单校裁剪**：上游同时服务西电（XDU）与外部用户；本项目按"广西大学单校"处理，登录流、数据源、解析器全部对接 GXU 研究生系统（`yjsxt.gxu.edu.cn`）与统一认证。
- **不保留双校分支**：凡用户可见的 XDU 默认行为，直接替换为 GXU 行为，不做兼容层，也不提供切换开关。
- **数据源替换**：成绩 / 选课 / 校园网 / 课表缓存均按 GXU 场景重新实现，本地加密落盘。
- **品牌与分发独立**：应用名、图标、官网（[gxu.app](https://gxu.app)）、更新清单与签名密钥均为本项目自有。
- **更新机制**：通过自有远端清单 + RSA-SHA256 签名校验检查更新，不依赖上游分发渠道。

同时明确**没有**做的事：不追加上游的新功能，不与上游保持代码同步，上游的 App Store / F-Droid 发布与本项目无关。
