...
- 当前维护仓库 GitHub owner 为 `GaleBird`，仓库名已于 2026-09-14 由 `traintime_pda` 改为 `traintime-pda-gxu`；本地 `origin`、README、官网源码展示和应用内仓库链接统一使用 `https://github.com/GaleBird/traintime-pda-gxu`。旧地址 `GaleBird/traintime_pda` 仍会 301 跳转，但不要再新写。注意：上游 `BenderBlog/traintime_pda`、Android `applicationId`/iOS widget provider 里的 `io.github.benderblog.traintime_pda`、i18n 中"基于 traintime_pda 开源项目"等上游指代都不是本仓库地址，改名时不要连带修改。
...
- 2026-09-08 修复官网“长时间无人访问后首次进站显示失败态（暂不可用/总下载次数 --/校验失败）”：根因是 manifest 缓存 TTL 过期后首个请求**同步等回源**，而 VPS 的 Docker 网桥 MTU(1500) 大于宿主机出口 `enp1s0` MTU(1480)，容器出站 TLS 握手要 3~7s（大包被隧道丢弃靠 TCP 重传撑过；宿主机 curl 只要 170ms），超过 CF Worker 4s 超时 → 503 → 前端 `renderFailureState()`；“刷新即好”是因为首个请求已把缓存填热。修复（上游 gxu-app commit `a63ac7d`，VPS 已同步部署同字节版本）：`manifest-client.js` 改为 stale-while-revalidate——TTL 过期立即返回旧清单 + 后台单飞刷新（失败仅 console.warn，不影响响应；并发请求共享一次刷新），`server.js` 启动时预热缓存。修复后 TTL 过期首请求 0.01s（原 6.5s+）。MTU 不匹配本身还在（影响该 VPS 所有容器的出站首包），如需根治可在宿主机加 `iptables -t mangle -A FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu`（需持久化）；当前服务层已完全规避，未动网络层。改动前备份在 VPS `/root/manifest-client.js.bak-20260908`、`/root/server.js.bak-20260908`。
- 2026-09-08 全链路审计后续加固（上游 gxu-app commit `2544478`，VPS 已同步部署）：`stats-store.js` 写入改 temp+rename 原子替换 + 滚动 `.bak`，主文件损坏回退备份/空态——此前损坏的 `download-counts.json` 会同时砸挂 `/api/stats`(500) 和 `/download/*`(502，计数在 302 之前)；前端失败态新增 `unreachable` 状态（琥珀徽标“清单不可达”），不再把清单不可达误标成“校验失败”，静态资源版本串 bump `?v=20260908-1`。生产环境已注入损坏文件验证自愈。
- **busybox `wget --spider` 发的是 GET 不是 HEAD**（已实测，会在 gxu-app 容器里照常触发下载计数）。免计数验证 `/download/*` 只能：宿主机 `curl -I https://origin.gxu.app/download/<abi>` 或容器内 `node -e "fetch(...,{method:'HEAD'})"`。
- gxu-app 容器内路径映射：宿主机 `/opt/gxu-app/service -> /app/service`（ro）、`/opt/gxu-app/data -> /app/data`（rw）；在容器里改数据文件要用 `/app/data/...` 路径，写 `/opt/...` 会 ENOENT。
- App 端 `pda_service_session.dart` 的 manifest 拉取 Dio 现在带 8s connect/receive/send 超时（`_updateManifestTimeout`）：此前裸 `Dio()` 无超时，主源 dl.gxu.app 被黑洞挂起时回退源 `gxu.app/api/manifest` 永远轮不到；不要把这个 Dio 的超时去掉。审计确认无虞的项：R2 manifest `no-store` 与 APK `immutable` 头由 CI `r2_upload.py` 显式传入、Caddy/Worker 配置正常、App 下载计数 fire-and-forget（2s 超时、失败仅记日志不阻断下载）。
- debug 安装陷阱（2026-09-08）：手机 `3B1F56E9B8L7YW34` 上的 `.dev` 包若与本机 `~/.android/debug.keystore` 签名不一致（曾用其他环境装过 1.0.6-dev），`adb install -r` 报 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`，必须先 `adb uninstall io.github.benderblog.traintime_pda.dev` 再装（会清调试包本地数据，日常用的正式包不受影响）；2026-09-08 已重装为本机密钥签名的 1.0.7-dev。本地无 `android/key.properties`，debug 构建始终走默认调试密钥。手机存在 user 10（分身/工作资料），部分 `adb shell pm/dumpsys` 需加 `--user 0`。
...
- 本机 `D:\Android\.gradle\wrapper\dists\gradle-8.11.1-bin` 曾损坏（缺 kotlin-compiler-embeddable jar，报 IllegalArgumentException），删掉整个 dist 目录让 wrapper 重下即恢复。
- 2026-09-14 仓库改名 `GaleBird/traintime_pda` → `GaleBird/traintime-pda-gxu`（旧地址 301 跳转）。同步范围：本仓库 `fork_info.dart` / `README.md` / `website/public/index.html` / 本地 `git remote`，以及官网仓库 `GaleBird/gxu-app` 的 `public/index.html` 源码区链接与展示名（commit 71cc4a0，已 scp 到 VPS `/var/www/gxu.app/public/index.html` 并验证线上 200）。**改名时不要连带改**：上游 `BenderBlog/traintime_pda`、Android `applicationId` / iOS widget provider 的 `io.github.benderblog.traintime_pda`、i18n 中"基于 traintime_pda 开源项目"的指代。注意 `gxu-app` 工作区 checkout 是 CRLF 而 VPS 上是 LF，增量部署要走 `git show HEAD:public/index.html > 文件` 取 blob 再 scp，否则整文件换行符变化。
- 2026-09-14 GitHub 仓库 About 栏已补齐：Description 写入（GXU 研课表定位 + 多平台 + 官网）、Topics 写入 18 个（`gxu`/`guangxi-university`/`gxu-app`/`flutter`/`dart`/`course-schedule`/`timetable`/`campus`/`graduate`/`student`/`unofficial`/`android`/`ios`/`linux`/`windows`/`university`/`mpl`/`watermeter`）。GitHub topic 只允许小写字母/数字/连字符，`mpl-2.0` 因含点号会被 422 拒绝，用 `mpl`。设置方式：`printf 'protocol=https\nhost=github.com\n\n' | git credential fill` 取 token（scope 含 `repo`）后打 `PATCH /repos/{owner}/{repo}` 与 `PUT /repos/{owner}/{repo}/topics`，topics 必须带 `Content-Type: application/json`。
- 2026-09-14 **分支已统一到 `main`**：`standalone-main` 的历史与 `main` 本就同点（f2c3f8b2），已用 `git checkout -B main origin/main` 把本地 `main` 复位到远端，随后删除本地与远端 `standalone-main`。现在仓库只有一个分支 `main`（也是 GitHub 默认分支），`git ls-remote --heads origin` 只应看到 `refs/heads/main`。旧历史没丢，仍在 `legacy/main-before-standalone`（b0661cf6）与 `legacy-history-before-standalone` 标签上。**不要再新建长期并行分支**；`tool/create_standalone_history.ps1` 的默认分支名仍叫 `standalone-main`，重跑脚本时记得改名再推。
- 2026-09-14 **README 与 `docs/` 已按 GXU 线重写**（37b43431）。此前 README 零超链接零图片、免责声明埋在末尾，`docs/README.md` / `docs/faq.md` 是整篇上游西电内容（开篇"为西电学生设计"、版本号 v1.5.6+36、下载链指向上游 F-Droid/App Store、登录排错指向 `ehall.xidian.edu.cn`、含 XDU Planet 章节）。现在两份都是 GXU 面向。**注意 `docs/faq.md` 里写的是行为契约**：选课缓存 24h（`gxu_course_selection_session.dart:17`）、`force` 绕过缓存、远端失败回退缓存、损坏缓存丢弃重取、GXU 模式不再刷新考试/物理实验（`setting.dart:728` 的 `!isGxuMode` 分支）。改这些代码时要同步改 faq.md，否则文档开始骗人。`docs/README.md` 与 `docs/data_structure.md`、`docs/contributors.md` 中的上游内容保留（后者是署名义务），`docs/_config.yaml`（`theme: jekyll-theme-minimal`）是 Jekyll 时代残留，Pages 已关闭故无副作用，未删。
- 2026-09-14 `docs/xdyou_eula.md` **不要删**。它是上游 BenderBlog 的法律文件、非本项目创建，条款一字未改，只在开头加了一段来源说明（声明这是上游协议、其"西安电子科技大学/IDS 服务"等指代的是上游场景、本应用内不存在需要勾选同意的协议界面）。同理不要把它改写成"GXU 用户协议"。
- 2026-09-14 README 截图在 `docs/screenshots/*.jpg`（home / classtable / classroom / selection / network），取自官网 gxu.app 当前 GXU 版效果图，压到 900px 宽 / q82 共 204KB 入库（原图 1MB 的课表 PNG → 96KB）。用 Python PIL 处理：`pip install pyyaml` 那套环境即可，本机没有 ImageMagick。**UI 有实质改动后要重出这批图**；不要改成热链 gxu.app 的图片，官网改版后会变空图。
- 2026-09-14 `release_for_linux.yml` / `release_for_windows.yml` 的 `push: tags` 触发已删除（9d1547fa）。原 filter `"v[0-9]+.[0-9]+.[0-9]+"` 是**正则写法，但 GitHub Actions 的 tag filter 是 glob**：`[0-9]` 只匹配单字符、`+` 是字面量，所以从建仓起从未命中，两个工作流在 11 次 Actions 运行里各 0 次。现在它们只支持 `workflow_dispatch`。**只把 glob 改成 `v*` 不是修好**，会把静默失败换成每次发版必红——这两份配置还有：`tag_name: ${{ github.event.inputs.tag_name }}` 在 push 下恒为空、`secrets.RELEASE_TOKEN` 本仓库不存在（Actions secrets 只有 DO_SPACES_* / R2_* / SIGNING_* / UPDATE_MANIFEST_SIGNING_KEY）、`checkout` 原先不取 submodule 导致 `.flutter/` 缺失、ubuntu-latest 上没有 `7z`。待修项已写在这两份 yml 的 `on:` 上方注释里。对照正确的写法看 `release_for_android.yaml`：glob `v*` + 显式声明 `workflow_dispatch.inputs.tag_name`。
- 2026-09-14 README 删掉"与上游的差异"一节（5aac35de）：开头已有非官方/来源说明，Sources 一节也保留了 LICENSE 与上游文档存档指向，单列一节讲项目治理把读者注意力从"能干什么"拉走了。上游关系只在 `## 反馈` 的"上游相关问题"一条里保留必要指向。
- 2026-09-14 **上游残留审计结论**（面向"明显影响判断和使用"）：应用**运行期 UI 已干净**——GXU 模式下首页只渲染 `ScoreCard` + `GxuCourseSelectionCard`（`homepage.dart:51`）外加 UpdateCard/ClassTableCard/SchoolnetCard；`LibraryCard`/`ExperimentCard`/`ExamCard`/`ElectricityCard`/`SchoolCardInfoCard`/`ClassAttendanceCard` 全是**无引用的孤儿 widget**；工具箱只列校园网 + 空闲教室 + 4 个明确标注"（未完成）"的占位；学校选择器（`school_mode`/`school_xidian`）已从代码彻底移除，`login.school_xidian` 是死文案；`XduPlanetPage` 无任何外部入口（`lib/page/xdu_planet/*` 与 `xdu_planet_session.dart` 均为死代码）；`jc_captcha.dart` 只被 `lib/repository/xidian_ids/*` 引用，而 GXU 模式无 XDU 登录入口，故整条 XDU 会话层不可达。`lib/page/experiment/*` 的 `experiment.score_hint_1/2`（"不在 XDYou 分数识别库中"）同理不可达。
- **真正还挂着上游身份的是打包期元数据，不在 Dart 层**。按影响排序：(1) `android/app/build.gradle:65` 的 `applicationId "io.github.benderblog.traintime_pda"`（`namespace` 同值）——**这是唯一随 APK 出厂的身份**，与上游 XDYou 共用同一 app 槽位，所以 README 那句"若已装过上游 XDYou 请先卸载，签名不同无法覆盖安装"正是它导致的；**且它是发布后不可变的**：改 applicationId 会让存量用户无法覆盖升级（只能卸载重装、丢本地数据），应用内按 versionCode 的更新判断也会对他们失效。要改必须配一次"迁移公告 + 强制重新下载"。审计结论是**当前不动**。(2) `windows/runner/Runner.rc` 的 CompanyName/FileDescription/InternalName/OriginalFilename/ProductName 全是 `traintime_pda`，`LegalCopyright` 写的是 `All rights reserved.`——与 MPL-2.0 直接冲突，会出现在 exe 属性和 UAC 弹窗里；`windows/runner/main.cpp:30` 的窗口标题 `L"traintime_pda"` 是用户可见的标题栏。(3) `linux/CMakeLists.txt:10` 的 `APPLICATION_ID "io.github.benderblog.traintime_pda"`。(4) `linux/packaging/watermeter.appdata.xml` 整份是占位/错值：`<name>/<summary>` 都是 `watermeter`、`<description><p>szgg</p></description>`、`<url type="homepage">` 指上游仓库、`<developer id="io.github.lijy91"><name>benderblog</name>`、`<project_license>MIT</project_license>`（项目实为 MPL-2.0），`linux/packaging/appimage/make_config.yaml` 的 display_name/generic_name 也是 `watermeter`。(5) `ios/Runner/Info.plist:51` `CFBundleName` = `xdyou`（`CFBundleDisplayName` 已是 `GXU研课表`，正确），另注册了 `xdyou://` URL scheme（代码里未见使用），`lib/repository/preference.dart:42` 的 iOS App Group `group.xyz.superbart.xdyou` 是小组件共享容器、改了会丢数据。iOS/Linux/Windows 均不出预编译包（只有 Android 发），故这些属于"将来真出包前必修"。
- `assets/guide/README.html`（40KB 西电校园网使用说明：XDUNA、`stu-xdwlan`、`zfw.xidian.edu.cn`、西电 VPN、`@stu.xidian.edu.cn` 邮箱、支付宝充值西电校园卡）**既未在 `pubspec.yaml` 的 assets 里声明、也没有任何代码引用**——Flutter 只打包声明过的资源，所以它不进包、不影响用户，属仓库卫生问题。`.vscode/settings.json:21` 的 `cmake.sourceDirectory` 指向上游作者的 Mac 路径 `/Volumes/BenderBlog/Development/traintime_pda/linux`，任何人 clone 后开 CMake 扩展都会指向不存在的目录；同文件 `licenser.author` 仍是 `Traintime PDA Authours, originally by BenderBlog Rodriguez`（这条是署名，保留合理）。
- 保留不动的上游痕迹（**不要清理**）：所有 `// Copyright 2023-2025 BenderBlog Rodriguez and contributors` + `SPDX-License-Identifier: MPL-2.0` 文件头；`fork_info.dart` 的 `upstreamRepositoryUrl`（关于页"上游仓库"入口）；i18n 的 `setting.about_page.upstream_*` 系列与 `copyright_notice`（法律声明）；`setting.header_subtitle`（"基于 Traintime PDA / XDYou 的 GXU 版本"，属主动署名）；`toolbox.*` 文案已全部 GXU 化无需处理。
- 2026-09-14 **禁止添加 AI 署名 trailer**（用户明确要求）：提交信息与 PR 描述一律不写 `Co-Authored-By: Claude ...`、`🤖 Generated with [Claude Code]` 之类协作工具署名。原因是这些 trailer 会让 GitHub 的贡献者图谱把 `claude`（`claude@anthropic.com` → GitHub 用户 `claude`）列成仓库贡献者，而本仓库的贡献者应当只反映真人维护者。注意 GitHub 的**网页贡献者图谱会计入 `Co-Authored-By`，而 REST `/contributors` 接口只统计作者**——所以这两个视图会不一致，别拿 API 结果当"网页上没显示"的证据。Codex 从未出现在贡献者里，只是因为它默认不写这类 trailer，不是仓库有规则拦它。
- 2026-09-14 **全量改写了提交历史以清除上述 trailer**（58 条提交，`git filter-branch --msg-filter`，已 `--force-with-lease` 推送到 `origin/main`，远端 `main` = `4f872501`）。改写只动提交信息：58 条提交的 tree 哈希 1:1 未变、作者/邮箱/日期/主题行均未变，仓库根树 `1026d456` 前后一致。**本地备份分支 `legacy/main-with-coauthor-trailers`（474b322e）只作回滚用，绝不要 push**——推上去会让带 trailer 的旧提交重新可达，贡献者图谱里 `claude` 就会回来。工具脚本在 `.codex_tmp/`（已被 gitignore，仅本机存在）。
- 改写踩到的坑，将来若还要改写历史务必注意：(1) **改写 `main` 会孤立所有挂着旧的 release tag**，必须同步重钉，否则 tag 与 main 脱钩。本仓库 11 个 `v1.0.1+41`…`v1.0.7+51` 已按"新旧历史第 i 条提交互相对应"重钉（映射脚本 `.codex_tmp/repin_tags.sh`，重钉前校验 subject+tree 一致才动）。附注 tag 要用 `git cat-file tag "$ref" | sed "1s/^object .*/object $new/" | git mktag` 重建才能保住 tagger/日期/说明；**`sed` 必须写 `1s` 而不是 `0,/re/s//…/`**，后者空正则复用上一个正则会把 `object ` 字面量插回去、拼出坏 sha（报 `badObjectSha1`）。(2) 重钉本地 tag **之后** `--force-with-lease` 会因 lease 基准失效而对 tag 报 `stale info` 拒绝推送，要改成逐 ref 显式给旧远端值 `--force-with-lease=refs/tags/X:<旧sha>`（旧值先 `git ls-remote --tags origin` 存一份）。推 `main` 本身用 `--force-with-lease=main:<旧sha>` 可正常成功，因为基准是在改写前取的。(3) 生成文件（`linux/flutter/`、`windows/flutter/`）里只有 CRLF/LF 差异时 `git stash push` 会报 `No valid patches in input`，直接 `git checkout -- <那些目录>` 即可，Flutter 会重新生成。
- 改写后已核验：`/repos/GaleBird/traintime-pda-gxu/contributors?anon=1` 只剩 `GaleBird`(56) + 匿名(2，即 root)；GitHub 上 7 个 Release（`v1.0.2+45`…`v1.0.7+51`）全部保留、各 3 个 ABI 附件、`/releases/download/<tag>/app-arm64-v8a-release.apk` 返回 200（tag 名未变故直链不受影响）；11 个 release tag 全部解引用到 `origin/main` 上的提交。仓库里另有 28 个 `v0.1.0`…`v1.5.8` 的上游遗留 tag，**它们本次改写前就不在 main 上**（属上游自带历史线），不是改写造成的，无需处理。
- 2026-09-14 **推 tag 会触发发版 CI，改 tag 指向前必须先想清楚**（本次踩坑记录）：重钉那 11 个 tag 时，`v1.0.1+41/42/43` 三个在远端**当时并不存在**（历史上被删过），推送对 GitHub 而言是「新建 tag ref」，于是 `release_for_android.yaml` 的 `on: push: tags: v*` 连触发 3 次（run 34826431073 / 534 / 529）。**触发规则是：新建 tag ref 会触发，force-update 一个已存在的 tag ref 不会触发**（所以另外 8 个一条都没跑）。这 3 次都在 `Build APK` 步骤挂在 `Execution failed for task ':home_widget:compileReleaseKotlin'`——那几个 3 月的提交早于 2026-09-05 的 Kotlin `jvmTarget` 修复，本来就构建不出；关键是 `Release` 步骤被 **skipped**，所以没有 Create/覆盖 Release、没有传 R2、**线上 `dl.gxu.app/manifests/update.json` 没被动**（事后核验仍是 `v1.0.7+51`）。**比对结果无损失，但这是个真隐患**：若某个旧 tag 被误推且构建成功，会发布旧版本并改写线上 manifest，`update_build_number.dart` 的版本比较会让所有用户都看不到更新，等于更新通道静默失效。**以后再改写历史/重钉 tag，务必先问：远端是否已有该 tag ref？新建就会发版。** 已根治：`release_for_android.yaml` 新增 **`Verify Release Tag`** 步骤（Checkout 之后、装 JDK/Flutter 之前的第一道，故失败只需几秒而非 5 分钟），两道校验——(1) tag 必须等于 `v` + `pubspec.yaml` 的 `version`，挡「tag 打了但版本没 bump」的错配；(2) tag 的 `+build` 不得**小于**已发布的最大 build（读 `GET /releases`，跳过 draft，用严格 `<` 故相等允许），挡「误推旧 tag 且构建成功→创建旧版本 Release→改写线上 manifest→所有用户更新检查静默失效」。正常「删发重发」流程会先删旧 Release 使最大 build 回落，故不被误拦。改这个护栏前请跑 `.codex_tmp/guardtest/matrix.sh`（6 个场景：旧 tag 指向旧提交 / tag 与版本错配 / 当前版本重跑 / 正常发下一版 / 删发重发 / pubspec 缺 +build；已全通过）。注意该测试必须整份跑在 bash 里——Python 的 `subprocess` 把 Windows 路径交给 Git Bash 会 rc 127 找不到脚本，且临时目录落在中文用户名下同样会失败，夹具要放仓库内 ASCII 路径。
