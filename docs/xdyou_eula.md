...
> **这不是 GXU 研课表的用户协议。**
>
> 以下全文是**上游 XDYou / Traintime PDA 的原始最终用户许可协议**，在本仓库中**仅作为来源说明与授权依据保留**，内容一字未改。
>
> - 协议中提到的"西安电子科技大学""IDS 服务""体适能""物理实验服务""宿舍电量服务"等，指的是**上游软件**面向西电的场景，与广西大学无关。
> - 协议中协议第五条（隐私政策条款）对隐私数据采集方的描述，指的是**上游**的数据采集方；本仓库 GXU 维护线的数据处理方式见 [`faq.md`](faq.md#隐私与数据安全)。
> - GXU 研课表**不提供**独立的终端用户协议页面，应用内也没有需要用户勾选同意的协议界面。
> - 本仓库对代码本身的授权以文件头部的 `SPDX-License-Identifier` 与仓库根目录 [`LICENSE`](../LICENSE)（MPL-2.0 为主）为准。
